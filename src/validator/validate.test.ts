...
describe('AC 오탐 — 34턴 실사용에서 막힌 정상 문장 (2026-09-03)', () => {
  const 정상 = [
    '알림톡 발송 웹훅 완료 즉시 DB의 is_immutable 필드가 true로 업데이트된다',
    '결제 완료 직후 지연 없이 대상 MP3/MP4 파일의 브라우저 다운로드가 자동으로 시작되어야 한다',
    '선택된 옵션에 따른 정확한 결제 금액이 PG 결제창에 바인딩되어야 한다',
    '웹 뷰어 내에서 브라우저 이탈 없이 즉시 오디오를 스트리밍 재생할 수 있는 내장 플레이어가 상시 제공된다',
  ];

  it('전부 통과해야 한다', () => {
    for (const ac of 정상) expect(isTestableAC(ac), ac).toBe(true);
  });

  it('§6.2가 막으라고 한 문장은 여전히 막힌다 — 수정 과잉 방지', () => {
    for (const ac of [
      '사용자 경험을 극대화한다',
      '안정적으로 동작한다',
      '직관적인 UI를 제공한다',
    ]) {
      expect(isTestableAC(ac), ac).toBe(false);
    }
  });
});
