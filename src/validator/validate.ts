...
const OBSERVABLE_STEMS = [
  '표시', '노출', '출력', '표출', '렌더', '나타',
  '저장', '기록', '등록', '삭제', '제거', '복원', '갱신', '업데이트', '수정', '초기화', '동기화',
  '차단', '거부', '무시', '잠금', '해제', '차감', '충전', '적립',
  '반환', '응답', '전송', '발송', '수신', '호출', '재시도', '리다이렉트',
  '생성', '합성', '변환', '계산', '집계', '검증', '조회', '검색', '정렬', '필터',
  '활성화', '비활성화', '이동', '포함', '다운로드', '업로드', '복사', '내보내',
  // M7 실사용(34턴 문서)에서 정상 AC가 막혀 추가한 것들 — 2026-09-03
  '시작', '중단', '재생', '바인딩', '반영', '발급', '만료', '결제', '환불',
].join('|');
// `할|될|되는|하는`은 관형형이다 — "재생할 수 있는", "업데이트되는".
// 어간 목록이 좁으므로 관형형을 열어도 "안정적으로 동작하는"은 여전히 막힌다(동작은 어간이 아니다).
